...
export function TraitImportExport({
  ...
  const handleImportTraits = async (
    ...
    try {
      ...
      jsonData.forEach((trait: any, index: number) => {
        ...
        if (!trait.id || typeof trait.id !== "string") {
          throw new Error(
            `Trait at index ${index}: Missing or invalid 'id' field.`
          )
        }

        // 验证多语言对象结构
        const validateMultiLangObject = (
          obj: any,
          fieldName: string,
          isOptional = false
        ) => {
          if (!obj || typeof obj !== "object") {
            ...
            throw new Error(
              `Trait at index ${index}: '${fieldName}' must be an object.`
            )
          }
          if (
            ...
            throw new Error(
              `Trait at index ${index}: '${fieldName}' must have 'en', 'zh-CN', and 'default' string properties.`
            )
          }
        }
        ...
        if (!trait.result || typeof trait.result !== "object") {
          throw new Error(
            `Trait at index ${index}: 'result' must be an object.`
          )
        }
        ...
        Object.entries(trait.result).forEach(([key, value]: [string, any]) => {
          if (!value || typeof value !== "object") {
            throw new Error(
              `Trait at index ${index}: 'result.${key}' must be an object.`
            )
          }
          if (
            ...
            throw new Error(
              `Trait at index ${index}: 'result.${key}' must have 'en', 'zh-CN', and 'default' string properties.`
            )
          }
        })
        ...
        if (typeof trait.category !== "string") {
          throw new Error(
            `Trait at index ${index}: 'category' must be a string.`
          )
        }
        ...
        if (
          trait.referenceGenotypes !== undefined &&
          !Array.isArray(trait.referenceGenotypes)
        ) {
          throw new Error(
            `Trait at index ${index}: 'referenceGenotypes' must be an array if present.`
          )
        }

        if (
          trait.yourGenotypes !== undefined &&
          !Array.isArray(trait.yourGenotypes)
        ) {
          throw new Error(
            `Trait at index ${index}: 'yourGenotypes' must be an array if present.`
          )
        }

        if (typeof trait.formula !== "string") {
          throw new Error(
            `Trait at index ${index}: 'formula' must be a string.`
          )
        }
        ...
        if (
          ...
          throw new Error(
            `Trait at index ${index}: 'scoreThresholds' must be an object.`
          )
        }
        ...
        if (typeof trait.confidence !== "string") {
          throw new Error(
            `Trait at index ${index}: 'confidence' must be a string.`
          )
        }

        if (typeof trait.isDefault !== "boolean") {
          throw new Error(
            `Trait at index ${index}: 'isDefault' must be a boolean.`
          )
        }

        if (typeof trait.createdAt !== "string") {
          throw new Error(
            `Trait at index ${index}: 'createdAt' must be a string.`
          )
        }

        if (!Array.isArray(trait.reference)) {
          throw new Error(
            `Trait at index ${index}: 'reference' must be an array.`
          )
        }
      })
